...
public class TitleBar : System.Windows.Controls.Control
{
    ...
    {
        var message = (Dwmapi.WM)msg;

        if (
            message
            is not (
                Dwmapi.WM.NCHITTEST
                or Dwmapi.WM.NCMOUSELEAVE
                or Dwmapi.WM.NCLBUTTONDOWN
                or Dwmapi.WM.NCLBUTTONUP
            )
        )
        {
            return IntPtr.Zero;
        ...
        if (message == Dwmapi.WM.NCHITTEST && Header is UIElement headerUiElement)
        {
        ...
        switch (message)
        {
            case Dwmapi.WM.NCHITTEST when CloseWindowByDoubleClickOnIcon && _icon.IsMouseOverElement(lParam):
                ...
                handled = true;
                return (IntPtr)Dwmapi.WM_NCHITTEST.HTSYSMENU;
            case Dwmapi.WM.NCHITTEST when this.IsMouseOverElement(lParam) && !isMouseOverHeaderContent:
                handled = true;
                return (IntPtr)Dwmapi.WM_NCHITTEST.HTCAPTION;
            default:
                return IntPtr.Zero;
        }
    }
