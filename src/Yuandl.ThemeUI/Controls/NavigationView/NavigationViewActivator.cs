    ...
    public static FrameworkElement? CreateInstance(Type pageType, object? dataContext = null)
    {
        if (!typeof(FrameworkElement).IsAssignableFrom(pageType))
        {
            throw new InvalidCastException(
                $"PageType of the ${typeof(IPageViewItem)} must be derived from {typeof(FrameworkElement)}. {pageType} is not."
            );
        }
        ...
        ConstructorInfo emptyConstructor =
            ...
            ?? throw new InvalidOperationException(
                $"The {pageType} page does not have a parameterless constructor. If you are using {pageType.Name} do not navigate initially and don't use Cache or Precache."
            );
